...
We kick off our initial Nmap scan, with default scripts `-sC`, enmuerate versions `-sV`, we want to see verbose output as nmap scans so we set `-vv` and then we specify to output the results in all formats available to us with the `-oA` and set our name of our file `sauna-namp`, and then we specify the IP address for Sauna, our particular target.
...
| 88  |   kerberos-sec   | Microsoft Windows Kerberos|   TCP   |
| 389  |   ldap   | Microsoft Windows Active Directory LDAP |   TCP   |
| 3268  |   ldap   | Microsoft Windows Active Directory LDAP |   TCP   |
