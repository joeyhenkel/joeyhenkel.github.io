...
We kick off our initial Nmap scan, with default scripts `-sC`, enmuerate versions `-sV`, we want to see verbose output of the resulsts as Nmap scans so we set `-vv` and then we specify to output the results in all formats that Nmap makes available to us with the `-oA` and then specify the name of the output file `sauna-namp`. Then finally we specify the IP address for Sauna, 10.10.10.175, the Sauna target.
